...
def load_models(feature):
    ...
    with tf.Session() as sess:
        new_saver = tf.train.import_meta_graph('./models/text_classifier/model.ckpt.meta')
        new_saver.restore(sess, tf.train.latest_checkpoint('./models/text_classifier/'))
...
def get_class(query):
    ...
    count_vect = CountVectorizer(decode_error="replace", vocabulary=pickle.load(open("feature.pkl", "rb")))
