...
def main(unused_argv):
    ...
    X_train, X_test, y_train, y_test = train_test_split(train_features, intent, test_size = 0.33, random_state = 42)

    # import sys
    # sys.exit(0)
    # zero_numpy = np.zeros(30*30)
    # zero_numpy[:]
    train_features = np.pad(train_features, ((0,0), (0,39)), 'constant')
    train_features = train_features.astype('float16')
    print("check1")
    # Create the Estimator
    mnist_classifier = tf.estimator.Estimator(
        model_fn=cnn_model_fn, model_dir="./tmp3/mnist_convnet_model")
    print("check2")
